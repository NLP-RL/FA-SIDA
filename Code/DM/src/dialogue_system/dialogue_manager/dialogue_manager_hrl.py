...
class DialogueManager_HRL(object):
    ...
    def next(self, greedy_strategy, save_record, index):
        ...
        if action_type=="symptom"  and self.parameter.get("agent_id").lower() == "agenthrljoint" and self.parameter.get("disease_as_action") == False:
            #print('##############')
            #if master_action_index==9:
            #    print(master_action_index)  ??action_type????????master_action_index?9???
        ...
        if episode_over == True:
            ...
            slots_proportion_list.append(num_of_true_slots - 1)  #??current slot?????slot???
            slots_proportion_list.append(num_of_all_slots - 1)   #??current slot????????slot???,?request???
            slots_proportion_list.append(real_implicit_slots)    #?????goal ??????true slot???
            try:
    ...
    def record_prioritized_training_sample(self, state, agent_action, reward, next_state, episode_over, **kwargs):
        if self.parameter.get("agent_id").lower() in ["agenthrljoint",'agenthrljoint2']:
            lower_reward = kwargs.get("lower_reward")
            master_action_index = kwargs.get("master_action_index")
            TD_error = kwargs.get("TD_error")
            self.state_tracker.agent.record_prioritized_training_sample(state, agent_action, reward, next_state, episode_over, lower_reward, master_action_index, TD_error)                                                                        
        else:
