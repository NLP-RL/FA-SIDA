...
class RunningSteward(object):
        ...
        for index in range(0, epoch_number,1):
            ...
            if result["success_rate"] > self.best_result["success_rate"] and \
                ...
                if save_model is True:
                    ...
                    if self.parameter.get("agent_id").lower() in ["agenthrljoint", "agenthrljoint2",'agentdqn'] and self.parameter.get("disease_as_action") is False and \
                        self.parameter.get("classifier_type") == "deep_learning":
    ...
    def evaluate_model(self, dataset, index):
        ...
        if self.parameter.get("train_mode") == False and index == self.parameter.get("simulate_epoch_number") - 1:
            ...
            if self.parameter.get("disease_as_action") is False:
        ...
        if self.parameter.get("use_all_labels") == True and self.parameter.get("agent_id").lower() in ["agenthrljoint", "agenthrljoint2"] and self.parameter.get('train_mode') == False and self.parameter.get("save_disease_record"):

            pickle.dump(self.dialogue_manager.disease_record, open(self.parameter.get("disease_record_file") + '/disease_record.p', 'wb'))
    ...
    def __dump_performance__(self, epoch_index):
        ...
        outfile = open(os.path.join(performance_save_path,file_name), "wb")
        pickle.dump(file=outfile, obj=self.learning_curve)
        outfile.close()
