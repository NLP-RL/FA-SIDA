...
class AgentHRL_joint2(object):
    def __init__(self, action_set, slot_set, disease_symptom, parameter):
        ...
        if self.parameter.get("data_type")=='simulated':
            if self.parameter.get("dataset_type") == 'SD':
                ...
                self.input_size_dqn_all = {1: 314, 2: 349, 3: 284, 4: 309, 5: 314, 6: 359, 7: 334, 8: 274, 9: 354, 10: 239, 11: 244, 12: 264}
        elif self.parameter.get("data_type") == 'real' and self.parameter.get("dataset_type") == 'SD':
        ...
        if parameter.get("state_reduced"):
            if parameter.get("wrong_disease_knowledge") or parameter.get("sf_idf_knowledge"):
                input_size = (len(self.slot_set) - 1 + len(self.disease_symptom.keys())) * 3   
            else:
                input_size = (len(self.slot_set) - 1) * 3
        else:
        ...
        print("-" * 10, " MASTER ", "-" * 10)
        print("Input Layer Size, Hidden Layer Size, Output Layer Size : ", input_size, hidden_size, self.output_size)
    ...
    def record_prioritized_training_sample(self, state, agent_action, reward, next_state, episode_over, lower_reward, master_action_index, TD_error):
        ...
        if int(agent_action) >= 0:
            ...
            #print(q_value)
            #print('# ', lower_reward)
            self.id2lowerAgent[self.current_lower_agent_id].record_prioritized_training_sample(state, agent_action, lower_reward, next_state, episode_over, TD_error)
        ...
        if self.subtask_terminal or int(agent_action) == -1 or episode_over==True:
            ...
            if self.master_action_index > (len(self.id2lowerAgent) - 1):
                ...
                if self.subtask_turn == 0:
                    ...
                    subtask_turn = self.subtask_turn
            #print(subtask_turn)
            self.experience_replay_pool.add_for_master(master_state_rep, master_action_index, self.master_reward, next_state_rep, episode_over, subtask_turn, TD_error)
            self.master_reward = 0
