...
class AgentDQN(Agent):
    def __init__(self, action_set, slot_set, disease_symptom, parameter, disease_as_action=True):
        ...
        if self.symptom_dist_as_input is True and self.agent_id.lower() == 'agenthrl':
            ...
            temp_slot_set = copy.deepcopy(slot_set)
            temp_slot_set.pop('disease')
            master_disease_symptom = pickle.load(open(parameter.get("disease_symptom"), 'rb'))
            if parameter.get("wrong_disease_knowledge") or parameter.get("sf_idf_knowledge"):
                input_size = (len(temp_slot_set) + len(master_disease_symptom.keys())) * 3
            else:
        ...
        print("Input Layer Size, Hidden Layer Size, Output Layer Size : ", input_size, hidden_size, output_size)
    ...
    def record_prioritized_training_sample(self, state, agent_action, reward, next_state, episode_over, TD_error):
        ...
        if self.parameter.get("state_reduced"):
            ...
            next_state_rep = state_to_representation_last(state=next_state, action_set=self.action_set,
                                                      ...
                                                      max_turn=self.parameter["max_turn"])
        self.experience_replay_pool.add_for_worker(state_rep, agent_action, reward, next_state_rep, episode_over, TD_error)
        self.action_visitation_count.setdefault(agent_action, 0)
        self.action_visitation_count[agent_action] += 1
