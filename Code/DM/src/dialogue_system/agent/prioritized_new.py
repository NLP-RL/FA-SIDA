...
class PrioritizedReplayBuffer(object):
    ...
    def add_for_worker(self, state, action, reward, next_state, episode_over, error):
        self._priorities.append((state, action, reward, next_state, episode_over, error))

    def add_for_master(self, state, action, reward, next_state, episode_over, subtask_turn, error):
        self._priorities.append((state, action, reward, next_state, episode_over, subtask_turn, error))

    def sample(self, batch_size, priority_scale=1.0):
        batch_size = min(len(self._priorities), batch_size)
        batch_probs = self.get_probabilities(priority_scale)
        batch_sample_tuple_size = len(self._priorities[0])
        ...
        batch = [self._priorities[x][:batch_sample_tuple_size-1] for x in batch_indices]

        return batch

    def get_probabilities(self, priority_scale):
        batch_sample_tuple_size = len(self._priorities[0])
        td_errors = np.array([abs(x[batch_sample_tuple_size-1]) for x in self._priorities])
        #print(td_errors)
